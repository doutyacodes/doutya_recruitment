...
const ChallengeBuzzWorld = ({
  ...
  const domains = item.task_keyword.domain
      ...
      <div className="rounded-md min-w-72">
        <div className={"px-3 py-1"}>
          <div className={"relative w-full h-24 md:h-56 border rounded-lg"}>
            {item.page_type == "internship" && (
              <div className="w-full h-full absolute left-0 top-0 z-50 bg-gray-400/70"></div>
            )}
            <Image
              src={imageSrc}
              fill
              alt="Profile Image"
              className="rounded-lg min-[576px]:object-cover md:object-contain max-[575px]:object-fill border border-white shadow-md"
            />
          </div>
        </div>
        <div className="w-full px-3">
          <div className="w-full md:flex justify-between items-center">
            <p
              className={cn(
                "font-bold text-[16px] my-1 whitespace-nowrap text-red-600 truncate"
              )}
            >
          ...
          <div className="grid grid-cols-12 min-h-12">
            ...
            <div className="col-span-4 w-full text-center">
              <p className="text-xs md:text-sm font-light">
                {item.page_type === "job" ? "Salary" : "Stipend"}
              </p>
              <p className="text-[10px] font-extrabold text-slate-600">
                {item.salary}
              </p>
